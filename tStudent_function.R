...
llh_mf_t <-
  function(df, x, y, low.freq, mu, omega, alpha, beta, gamma,
    ...
    if (sum(g <= 0) > 0) {
      ...
      # edited
      ll<-ll <- -dstd(ret, mean = mu, sd = g*tau, nu = nu, log = TRUE)
    }
  }

llh_simple_t <- function(y, mu, alpha, beta, gamma, m, nu, g_zero) {
  ...
  # t-Student distribution
  ll<-ll <- -dstd(ret, mean = mu, sd = g*exp(m), nu = nu, log = TRUE)
}

##fit_mfgarch edited with t-Student
...
fit_mfgarch_t <- function(data, y, x = NULL, K = NULL, low.freq = "date", var.ratio.freq = NULL, gamma = TRUE, weighting = "beta.restricted", x.two = NULL, K.two = NULL, low.freq.two = NULL, weighting.two = NULL, multi.start = FALSE, control = list(par.start = NULL)) {
  ...
  if (K == 0) {
    if (gamma == TRUE) {
      ...
      par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04, m = 0, nu = 3)
      ui.opt <- rbind(c(0, -1, -1, -1/2, 0, 0), c(0, 1, 0, 0, 0, 0), 
                      c(0, 0, 1, 0, 0, 0), c(0, 0, 0, 0, 0, 1))
      ci.opt <- c(-0.99999, 0, 2.001)
    } else {
      lf <- function(p) {
        llh_simple_t(y = ret,
                     mu = p["mu"],
      ...
      par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, m = 0, nu=3)
      ui.opt <- rbind(c(0, -1, -1, 0, 0), c(0, 1, 0, 0, 0), 
                      c(0, 0, 1, 0, 0), c(0, 0, 0, 0, 1))
      ci.opt <- c(-0.99999, 0, 0, 2.001)
    }
  ...
  if (K == 1) {
    if (is.null(K.two) == FALSE) {
      if (gamma == TRUE) {
        ...
        par_start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04, m = 0, theta = 0, theta.two = 0, nu = 3)
        ui_opt <- rbind(c(0, -1, -1, -1/2, 0, 0, 0, 0), c(0, 1, 0, 0, 0, 0, 0, 0), 
                        c(0, 0, 1, 0, 0, 0, 0, 0), c(0, 0, 0, 0, 0, 0, 0, 1))
        ci_opt <- c(-0.99999, 0, 0, 2.001)
      } else {
        lf <- function(p) {
          llh_mf_t(df = df_llh,
        ...
        par_start <- c(mu = 0, alpha = 0.02, beta = 0.85, m = 0, theta = 0, theta.two = 0, nu = 3)
      ...
      if (gamma == TRUE) {
        ...
        par_start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04, m = 0, theta = 0, nu = 3)
        ...
        par_start <- c(mu = 0, alpha = 0.02, beta = 0.85, m = 0, theta = 0, nu = 3)
  ...
  if (K > 1) {
    if (gamma == TRUE) {
      if (weighting == "beta.restricted" & is.null(K.two) == TRUE) {
        ...
        par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04,
                       m = 0, theta = 0, w2 = 3, nu = 3)
        ui.opt <- rbind(c(0, -1, -1, -1/2, 0, 0, 0, 0),
                        c(0,  0,  0,    0, 0, 0, 1, 0),
                        c(0,  1,  0,    0, 0, 0, 0, 0),
                        c(0,  0,  1,    0, 0, 0, 0, 0),
                        c(0,  0,  0,    0, 0, 0, 0, 1))
        ci.opt <- c(-0.99999999, 1, 0, 0, 2.001)
      }
      if (weighting == "beta.restricted" & is.null(K.two) == FALSE) {
        if (K.two == 1) {
          ...
          par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04,
                         m = 0, theta = 0, w2 = 3, theta.two = 0, nu = 3)
          ui.opt <- rbind(c(0, -1, -1, -1/2, 0, 0, 0, 0, 0),
                          c(0,  0,  0,    0, 0, 0, 1, 0, 0),
                          c(0,  1,  0,    0, 0, 0, 0, 0, 0),
                          c(0,  0,  1,    0, 0, 0, 0, 0, 0), 
                          c(0,  0,  0,    0, 0, 0, 0, 0, 1))
          ci.opt <- c(-0.99999999, 1, 0, 0, 2)
        }
        if (K.two > 1) {
          if (weighting.two == "beta.restricted") {
            lf <- function(p) {
            ...
            par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04,
                           m = 0, theta = 0, w2 = 3, theta.two = 0, w2.two = 3, nu = 3)
      ...
      if (weighting == "beta.unrestricted" & is.null(K.two) == TRUE){
        ...
        par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04,
                       m = 0, theta = 0, w1 = 1.0000001, w2 = 3, nu = 3)
        ui.opt <- rbind(c(0, -1, -1, -1/2, 0, 0, 0, 0, 0),
                        c(0,  0,  0,  0,   0, 0, 1, 0, 0),
                        c(0,  0,  0,  0,   0, 0, 0, 1, 0),
                        c(0,  1,  0,  0,   0, 0, 0, 0, 0),
                        c(0,  0,  1,  0,   0, 0, 0, 0, 0),
                        c(0,  0,  0,  0,   0, 0, 0, 0, 1))
        ci.opt <- c(-0.99999999, 1, 1, 0, 0, 2.001)
      }
      ...
      if (weighting == "beta.unrestricted" & is.null(weighting.two) == FALSE) {
        if (weighting.two == "beta.restricted") {
          ...
          par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04,
                         m = 0, theta = 0, w1 = 1.00000001, w2 = 3, theta.two = 0, w2.two = 3, nu = 3)
          ...
          ci.opt <- c(-0.99999999, 1, 1, 0, 0, 1, 2.001)
        }
      }
    }
    ...
    if (gamma == FALSE) {
      ...
      if (weighting == "beta.restricted") {
        ...
        par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, m = 0, theta = 0, w2 = 3, nu = 3)
      ...
      if (weighting == "beta.unrestricted") {
        ...
        par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, m = 0, theta = 0, w1 = 1.00000001, w2 = 3, nu = 3)
...
fit_t_dhousing$par
fit_t_dhousing$bic
fit_t_dinpro<-fit_mfgarch_t(data = v, y = "return", x = "dindpro", K = 36, low.freq = "year_month")
fit_t_dinpro$par
fit_t_dinpro$bic
fit_t_nai<-fit_mfgarch_t(data = v, y = "return", x = "nai", K = 36, low.freq = "year_month")
fit_t_nai$par
fit_t_nai$bic
fit_t_nfci<-fit_mfgarch_t(data = v, y = "return", x = "nfci", K = 52, low.freq = "year_week")
fit_t_nfci$par
fit_t_nfci$bic

par_dhousing_t<-fit_prova$broom.mgarch[c(2,3)]
par_dinpro_t<-fit_t_dinpro$broom.mgarch[c(2,3)]
par_nai_t<-fit_t_dinpro$broom.mgarch[c(2,3)]
par_nfci_t<-fit_t_nfci$broom.mgarch[c(2,3)]

par_fit<-t(cbind(par_dhousing_t, par_dinpro_t, par_nai_t, par_nfci_t))
