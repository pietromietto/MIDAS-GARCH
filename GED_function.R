...
# Dataset
v<-data.frame(df_mfgarch[1:11644,1:11])

# Upload useful spackages
# refer to this link
# https://cran.r-project.org/web/packages/mfGARCH/index.html

# GED log-likelyhood
llh_mf_GED <-
  function(df, x, y, low.freq, mu, omega, alpha, beta, gamma,
           m, nu, theta, w1 = 1, w2 = 1, g_zero, K,
    ...
    if (sum(g <= 0) > 0) {
      ...
      # GED distribution
      ll<-ll <- -dged(ret, mean = mu, sd = g*tau, nu = nu, log = TRUE)
    }
  }

llh_simple_GED <- function(y, mu, alpha, beta, gamma, m, nu, g_zero) {
  ...
  # GED distribution
  # mfGarch edited
  ll<-ll <- -dged(ret, mean = mu, sd = g*exp(m), nu = nu, log = TRUE)
}

## mfgarch assuming GED distribution

fit_mfgarch_GED <- function(data, y, x = NULL, K = NULL, low.freq = "date", var.ratio.freq = NULL, gamma = TRUE, weighting = "beta.restricted", x.two = NULL, K.two = NULL, low.freq.two = NULL, weighting.two = NULL, multi.start = FALSE, control = list(par.start = NULL)) {
  ...
  if (K == 0) {
    if (gamma == TRUE) {
      ...
      par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04, m = 0, nu = 0.02)
      ui.opt <- rbind(c(0, -1, -1, -1/2, 0, 0), c(0, 1, 0, 0, 0, 0), 
                      c(0, 0, 1, 0, 0, 0), c(0, 0, 0, 0, 0, 1))
      ci.opt <- c(-0.99999, 0, 0, 0.01)
    } else {
      lf <- function(p) {
        llh_simple_GED(y = ret,
                     mu = p["mu"],
      ...
      par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, m = 0, nu=0.02)
      ui.opt <- rbind(c(0, -1, -1, 0, 0), c(0, 1, 0, 0, 0), 
                      c(0, 0, 1, 0, 0), c(0, 0, 0, 0, 1))
      ci.opt <- c(-0.99999, 0, 0, 0.01)
    }
  ...
  if (K == 1) {
    if (is.null(K.two) == FALSE) {
      if (gamma == TRUE) {
        lf <- function(p) {
          llh_mf_GED(df = df_llh, y = ret, x = covariate, low.freq = low.freq,
                   mu = p["mu"],
        ...
        par_start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04, m = 0, theta = 0, theta.two = 0, nu = 0.02)
        ui_opt <- rbind(c(0, -1, -1, -1/2, 0, 0, 0, 0), c(0, 1, 0, 0, 0, 0, 0, 0), 
                        c(0, 0, 1, 0, 0, 0, 0, 0), c(0, 0, 0, 0, 0, 0, 0, 1))
        ci_opt <- c(-0.99999, 0, 0, 0.01)
      } else {
        lf <- function(p) {
          llh_mf_GED(df = df_llh,
        ...
        par_start <- c(mu = 0, alpha = 0.02, beta = 0.85, m = 0, theta = 0, theta.two = 0, nu = 0.02)
        ui_opt <- rbind(c(0, -1, -1,  0, 0, 0, 0), c(0, 1, 0, 0, 0, 0, 0), 
                        c(0, 0, 1, 0, 0, 0, 0), c(0, 0, 1, 0, 0, 0, 1))
        ci_opt <- c(-0.99999, 0, 0, 0.01)
      }
      ...
      if (gamma == TRUE) {
        ...
        par_start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04, m = 0, theta = 0, nu = 0.02)
        ui_opt <- rbind(c(0, -1, -1, -1/2, 0, 0, 0), c(0, 1, 0, 0, 0, 0, 0), 
                        c(0, 0, 1, 0, 0, 0, 0), c(0, 0, 0, 0, 0, 0, 1))
        ci_opt <- c(-0.99999, 0, 0, 0.01)
      } else {
        lf <- function(p) {
          llh_mf_GED(df = df_llh,
        ...
        par_start <- c(mu = 0, alpha = 0.02, beta = 0.85, m = 0, theta = 0, nu = 0.02)
        ui_opt <- rbind(c(0, -1, -1,  0, 0, 0), c(0, 1, 0, 0, 0, 0), 
                        c(0, 0, 1, 0, 0, 0), c(0, 0, 0, 0, 0, 1))
        ci_opt <- c(-0.99999, 0, 0, 0.01)
      }
    }
  ...
  if (K > 1) {
    if (gamma == TRUE) {
      if (weighting == "beta.restricted" & is.null(K.two) == TRUE) {
        ...
        par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04,
                       m = 0, theta = 0, w2 = 3, nu = 0.02)
        ui.opt <- rbind(c(0, -1, -1, -1/2, 0, 0, 0, 0),
                        c(0,  0,  0,    0, 0, 0, 1, 0),
                        c(0,  1,  0,    0, 0, 0, 0, 0),
                        c(0,  0,  1,    0, 0, 0, 0, 0),
                        c(0,  0,  0,    0, 0, 0, 0, 1))
        ci.opt <- c(-0.99999999, 1, 0, 0, 0.01)
      }
      if (weighting == "beta.restricted" & is.null(K.two) == FALSE) {
        if (K.two == 1) {
          ...
          par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04,
                         m = 0, theta = 0, w2 = 3, theta.two = 0, nu = 0.02)
        ...
        if (K.two > 1) {
          if (weighting.two == "beta.restricted") {
            ...
            par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04,
                           m = 0, theta = 0, w2 = 3, theta.two = 0, w2.two = 3, nu = 0.02)
            ...
            ci.opt <- c(-0.99999999, 1, 0, 0, 1, 0.01)
          }
          if (weighting.two != "beta.restricted") {
      ...
      if (weighting == "beta.unrestricted" & is.null(K.two) == TRUE){
        lf <- function(p) {
          llh_mf_GED(df = df_llh,
                   ...
                   omega = 1 - p["alpha"] - p["beta"] - p["gamma"]/2,
                   alpha = p["alpha"], beta = p["beta"], gamma = p["gamma"],
                   m = p["m"], theta = p["theta"], nu=p["nu"], w1 = p["w1"], w2 = p["w2"],
                   g_zero = g_zero,
                   K = K)
        }
        par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04,
                       m = 0, theta = 0, w1 = 1.0000001, w2 = 3, nu = 0.02)
        ui.opt <- rbind(c(0, -1, -1, -1/2, 0, 0, 0, 0, 0),
                        c(0,  0,  0,  0,   0, 0, 1, 0, 0),
                        c(0,  0,  0,  0,   0, 0, 0, 1, 0),
                        c(0,  1,  0,  0,   0, 0, 0, 0, 0),
                        c(0,  0,  1,  0,   0, 0, 0, 0, 0),
                        c(0,  0,  0,  0,   0, 0, 0, 0, 1))
        ci.opt <- c(-0.99999999, 1, 1, 0, 0, 0.01)
      }
      ...
      if (weighting == "beta.unrestricted" & is.null(weighting.two) == FALSE) {
        if (weighting.two == "beta.restricted") {
          ...
          par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, gamma = 0.04,
                         m = 0, theta = 0, w1 = 1.00000001, w2 = 3, theta.two = 0, w2.two = 3, nu = 0.02)
          ui.opt <- rbind(c(0, -1, -1, -1/2, 0, 0, 0, 0, 0, 0, 0),
          ...
          ci.opt <- c(-0.99999999, 1, 1, 0, 0, 1, 0.01)
        }
      }
    }
    ...
    if (gamma == FALSE) {
      ...
      if (weighting == "beta.restricted") {
        lf <- function(p) {
          llh_mf_GED(df = df_llh,
                   ...
                   m = p["m"], theta = p["theta"], nu["nu"], w1 = 1, w2 = p["w2"],
                   g_zero = g_zero,
                   K = K)
        }
        par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, m = 0, theta = 0, w2 = 3, nu = 0.02)
      ...
      if (weighting == "beta.unrestricted") {
        ...
        par.start <- c(mu = 0, alpha = 0.02, beta = 0.85, m = 0, theta = 0, w1 = 1.00000001, w2 = 3, nu = 0.02)
        ui.opt <- rbind(c(0, -1, -1, 0, 0, 0, 0, 0),
                        c(0,  0,  0, 0, 0, 1, 0, 0),
                        c(0,  0,  0, 0, 0, 0, 1, 0),
                        c(0,  1,  0, 0, 0, 0, 0, 0),
                        c(0,  0,  1, 0, 0, 0, 0, 0),
...
fit_GED_dhousing$par
fit_prova_GED$bic
fit_GED_dinpro<-fit_mfgarch_GED(data = v, y = "return", x = "dindpro", K = 36, low.freq = "year_month")
fit_GED_dinpro$par
fit_GED_dinpro$bic
fit_GED_nai<-fit_mfgarch_GED(data = v, y = "return", x = "nai", K = 36, low.freq = "year_month")
fit_GED_nai$par
fit_GED_nai$bic
fit_GED_nfci<-fit_mfgarch_GED(data = v, y = "return", x = "nfci", K = 52, low.freq = "year_week")
fit_GED_nfci$par
fit_GED_nfci$bic

par_dhousing_GED<-fit_GED_dhousing$broom.mgarch[c(2,3)]
par_dinpro_GED<-fit_GED_dinpro$broom.mgarch[c(2,3)]
par_nai_GED<-fit_GED_nai$broom.mgarch[c(2,3)]
par_nfci_GED<-fit_GED_nfci$broom.mgarch[c(2,3)]

par_fit_GED<-t(cbind(par_dhousing_GED, par_dinpro_GED, par_nai_GED, par_nfci_GED))
nomi_fit<-t(c("dhousing","dhousing", "dinpro", "dinpro", "NAI", "NAI", "NFCI", "NFCI"))
xtable(cbind(t(nomi_fit), round(par_fit_GED, 3)))
